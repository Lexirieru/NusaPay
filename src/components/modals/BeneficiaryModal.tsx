  ...
  const handleSubmit = async () => {
    ...
    if (isEditMode && recipient) {
      ...
      const editRec = {
        ...
        _id: recipient.id, // Keep the same ID
        bankCode: "014", // Ganti dengan kode bank yang sesuai pake enum (misalnya BCA = 014)
        bankAccount: formData.account,
        bankAccountName: formData.bank,
        amountTransfer: Number.parseFloat(formData.amount),
      };
      onSave(updateRecipient);
      await editEmployeeData(editRec);
    } else {
      ...
      const newRec = {
        ...
        companyId: "685c215590a60ac5d5672cd4", // Ganti dengan ID perusahaan yang sesuai (dari google auth)
        companyName: "Company Name", // Ganti dengan nama perusahaan yang sesuai
        name: formData.name,
        bankCode: "014", // Ganti dengan kode bank yang sesuai (misalnya BCA = 014)
        bankAccount: formData.account,
        bankAccountName: formData.bank, //BCA?
        walletAddress: "12314", // Ganti dengan alamat dompet yang sesuai
        networkChainId: 1, // Ganti dengan ID jaringan yang sesuai (misalnya Ethereum = 1)
        amountTransfer: Number.parseInt(formData.amount),
      };
      onSave(newRecipient);
      await addEmployeeData(newRec);
    }
  };
