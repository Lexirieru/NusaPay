...
export async function sendToken(recipient: string, amount: string, PRIVATE_KEY : string, txId: string, userId : string) {
const wallet = new ethers.Wallet(PRIVATE_KEY, provider);
// udah connect ke smart contract USDC di base dan connect ke walletku
const usdc = new ethers.Contract(usdcContractAddress, usdcABI, wallet);
...
export async function doMinting(txId : string, userId : string, IDRX_API_KEY : string, IDRX_SECRET_KEY : string, PRIVATE_KEY : string, amount : string){
